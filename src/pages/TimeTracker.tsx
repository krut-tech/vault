...
export default function TimeTracker() {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="glass-panel glow-border p-6 text-center">
        ...
        {!running ? (
          <div className="mt-4 flex gap-2 max-w-sm mx-auto">
            <input className="input-field flex-1" placeholder="What are you working on?" value={note} onChange={(e) => setNote(e.target.value)} />
            <button onClick={handleStart} className="btn-primary flex items-center gap-1.5 text-sm px-4">
              <Play size={14} /> Start
            </button>
          </div>
        ) : (
          <button onClick={handleStop} className="mt-4 px-5 py-2 rounded-xl bg-magenta/20 text-magenta border border-magenta/40 hover:bg-magenta/30 flex items-center gap-1.5 mx-auto text-sm">
            <Square size={14} /> Stop
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="glass-panel p-4 text-center">
          <p className="text-xl font-bold text-cyan">{formatDuration(todayTotal)}</p>
          <p className="text-xs text-gray-500 mt-1">Today</p>
        </div>
        <div className="glass-panel p-4 text-center">
          <p className="text-xl font-bold text-violet">{formatDuration(allTotal)}</p>
          <p className="text-xs text-gray-500 mt-1">All logged time</p>
        </div>
      </div>
      ...
      <section className="glass-panel">
        ...
        <div className="divide-y divide-white/5 max-h-96 overflow-y-auto">
          {entries.length === 0 && <p className="px-5 py-4 text-sm text-gray-500">No entries yet.</p>}
          {entries.map((e) => (
            <div key={e.id} className="px-5 py-2.5 text-sm flex items-center justify-between">
              <div>
                <p>{e.note || 'Untitled session'}</p>
                <p className="text-xs text-gray-500">{format(new Date(e.started_at), 'MMM d, HH:mm')}</p>
              </div>
